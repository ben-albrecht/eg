...
awk -v "n=$LineNumber" '(NR==n) {print $1}'
...
awk 'length($0) > 80'
#This program prints every line longer than 80 characters.
#   The sole rule has a relational expression as its pattern,
...
#   (or rather, to create a new file similar to the old file but from 
#   which the blank lines have been deleted).

awk '{ if (NF > 0) print }'
#This program also prints every line that has at least one field.
#   Here we allow the rule to match every line, then decide in the action whether to print.
...
#This program prints the maximum line length of file.
#   The input is piped through the expand program to change tabs into spaces,
